...
<div class="container">
  ...
  ?>
      <div class="w3-third">
        ...
        <a href="#"><img src="<?= $row['image'] ?>" id="margin" style="width: 65%"></a>
         <h2><?= $row['name'] ?></h2></a>
      </div>
       <?php }
      } ?>
  ...
  require_once('conn.php');
       ...
       $maphim = $_GET['maphim'];
      $sql = "SELECT thoigian FROM lichchieu where maphim=".$maphim;
      $result = $conn->query($sql);
      if($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
  ?>

 <a href="datghe.php"><button class="btn btn-danger"><?= $row['thoigian'] ?></button></a>
      <?php }
      } ?>
    </div>
